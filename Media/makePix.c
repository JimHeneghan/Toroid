...
int main()
{
  ...
  printf("---> %f\n", MaxVal);
   ...
   while (i < MAX){
     ...
     //fread(&temp, sizeof(float), 1, source);
     x[i] = fabs(temp);
     //The following if else statement creates greater image resolution for field strengths of less than
     //half the maximum value. All values greater than Maxval/2 are set to the maximum
/* 
    if(x[i] > (MaxVal/2))
     	ix[i] = (int) (CMAX);
     else
     	ix[i] = (int) (x[i]*CMAX*2)/MaxVal;
*/  
   ...
   sprintf(filenamedest, "Square.pgm");
