...
void gridInit(Grid *g, double Kx, double Ky, double Kz) {
  ...
  a = 50;
  SizeX = 61*sqrt(3)*a; // size of domain
  SizeY = 2*a;
  SizeZ = 3;
  ...
  /* set cylinders*/
  /*setting the coordinates for the left most grid points in each unit cell*/
  /*this lets us locate the cell as a function of the lattice vector a*/
  for (i = 0; i< 62; i++){
    for (mm = 0; mm < SizeX; mm++) /*@ \label{grid3dhomoE} @*/
      for (nn = 0; nn < SizeY; nn++) 
	for (pp = 0; pp < SizeZ; pp++) {
	  ...
	  XLoc3 = XCenter1 - a*sqrt(3)/2 + i*a*sqrt(3) - mm;
